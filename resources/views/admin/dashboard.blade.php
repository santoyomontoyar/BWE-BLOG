...
<div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
    ...
    <div class="flex justify-between items-center mb-8 pb-6 border-b border-gray-100">
        ...
        <div class="flex items-center space-x-6">
            ...
            <div class="text-sm text-gray-600 flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-full border border-gray-100">
                ...
                <a href="{{ route('language.switch', 'en') }}" class="font-semibold {{ $currentLang == 'en' ? 'text-blue-700 bg-blue-100 px-3 py-0.5 rounded-full text-xs' : 'text-gray-500 hover:text-gray-800 px-3 py-0.5 rounded-full text-xs transition' }}">GB English</a>
            </div>
    ...
    <tr class="hover:bg-blue-50/50 transition-colors group">
        <!-- Columna Número -->
        <td class="py-6 px-5 font-semibold text-gray-800 align-top">
            {{ $posts->firstItem() + $index }}
        </td>
        
        <!-- Columna Título -->
        <td class="py-6 px-5 align-top overflow-hidden">
            <p class="font-bold text-gray-900 leading-snug group-hover:text-blue-800 line-clamp-2">
                {{ $post->display_title }}
            </p>
        </td>
    ...
    <div class="flex items-center justify-between mt-8 pt-6 border-t border-gray-100 text-sm text-gray-600">
        <p>
            Mostrando 
            <span class="font-semibold text-gray-800">{{ $posts->firstItem() ?? 0 }}</span> a 
            <span class="font-semibold text-gray-800">{{ $posts->lastItem() ?? 0 }}</span> de 
            <span class="font-semibold text-gray-800">{{ $posts->total() }}</span> entradas
        </p>
